...
const { loadFirstSeen, NOT_EXCLUDED, loadInsightCache, saveInsightCache, clearInsightCache } = require('../agg');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!readQueryParam(req, 'purge')) {
      ...
      // 서버리스라 인스턴스 메모리 캐시는 거의 안 맞는다. DB 캐시는 1행 읽기로 끝난다.
      const stored = await loadInsightCache(db, cacheKey);
      if (stored) {
    ...
    await saveInsightCache(db, cacheKey, payload);
