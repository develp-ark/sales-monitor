...
const { clearInsightCache } = require('../agg');
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // 인사이트의 '신규' 목록이 이 테이블을 읽는다.
    if (added) { try { await clearInsightCache(db); } catch (e) { /* 캐시 실패는 무시 */ } }
  ...
  if (req.method === 'DELETE') {
    ...
    if (removed) { try { await clearInsightCache(db); } catch (e) { /* 캐시 실패는 무시 */ } }
