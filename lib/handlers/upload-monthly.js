...
const { touchFirstSeen, touchArchiveMeta, clearInsightCache } = require('../agg');
  ...
  try {
    ...
    for (const item of items) {
      ...
      for (const [month, sales] of Object.entries(item.months || {})) {
        ...
        metaRows.push({ brand, sku_id: String(item.sku_id), sku_name: item.sku_name || '', month });
        totalRows++;
    ...
    try {
      ...
      await touchArchiveMeta(db, metaRows);
      await clearInsightCache(db);   // 인사이트가 sales_monthly 를 읽는다
    } catch (e) {
