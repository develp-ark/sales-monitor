...
const { loadBrandDaily, loadSkuMonthBefore, loadBrands, loadArchiveSku, loadArchiveMonth } = require('../agg');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const [salesR, archiveR, archiveSku, archiveMonth, monthlyAgg, trendAgg, brandList] = await Promise.all([
      db.execute({ sql: salesSql, args: salesArgs }),
      // sales_monthly 는 4분의 3이 0 이다. 값은 비영 행만 읽고(부분 인덱스),
      // 0 행만 기여하던 월 축과 SKU 이름은 미리 만들어 둔 메타에서 가져온다.
      db.execute({
    ...
    for (const r of archiveMonth) monthAxis.set(r.brand + '|' + r.month, { brand: r.brand, month: r.month });
    for (const r of archiveSku) nameMap.set(r.brand + '|' + r.sku_id, { month: r.first_month, sku_name: r.sku_name });
