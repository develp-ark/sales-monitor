 ...
 * 브랜드 시트 동기화.
 *
 * 기존 시트를 읽어 skuMap 으로 만든 뒤 rows 를 병합하므로, 호출부는 브랜드 전체 이력이
 * 아니라 이번에 올라온 날짜의 행만 넘기면 된다 (DB 읽기 25만 -> 1천).
 * opts.fetchAll: 시트가 비어 있어 과거 이력을 복구해야 할 때만 쓰는 전체 조회 함수.
 */
async function syncBrandSheet(brandName, rows, opts) {
  ...
  const meta = await sheets.spreadsheets.get({
    ...
    fields: 'sheets(properties(sheetId,title,gridProperties),conditionalFormats(booleanRule))'
  });
  ...
  // 증분 동기화는 시트에 이미 빠져 있는 날짜를 메우지 못한다(동기화 실패나 수동 편집으로
  // 생긴 구멍이 0 으로 굳는다). 시트가 비었거나 기대 날짜 수보다 적으면 전체 이력으로 다시 쓴다.
  if (opts && opts.partial && opts.fetchAll) {
    const expected = opts.expectedDates;
    const missing = existingRows.length < 2
      || (typeof expected === 'number' && expected > 0 && existingDates.length < expected);
    if (missing) {
      const full = await opts.fetchAll();
      if (full && full.length) {
        console.log('[SHEETS] ' + brandName + ': 시트 날짜 ' + existingDates.length + '/' + expected
          + ' — 전체 이력 ' + full.length + '행으로 다시 씁니다');
        rows = full;
      }
    }
  }
  ...
  // clear를 먼저 하면 update가 실패했을 때 시트가 통째로 빈 채로 남는다.
  // 덮어쓰기를 먼저 하고, 이전 데이터가 더 넓었던 잔여 영역만 뒤에서 지운다.
  ...
  const prevRowCount = existingRows.length;
  const prevColCount = existingRows.reduce((m, r) => Math.max(m, r.length), 0);
  const leftoverRanges = [];
  if (prevRowCount > totalRows) {
    const lastCol = colToA1(Math.max(prevColCount, totalCols) - 1);
    leftoverRanges.push(`${brandName}!A${totalRows + 1}:${lastCol}${prevRowCount}`);
  }
  if (prevColCount > totalCols) {
    leftoverRanges.push(`${brandName}!${colToA1(totalCols)}1:${colToA1(prevColCount - 1)}${totalRows}`);
  }
  if (leftoverRanges.length) {
    await sheets.spreadsheets.values.batchClear({
      spreadsheetId: SPREADSHEET_ID,
      requestBody: { ranges: leftoverRanges }
    });
  }
  ...
  if (existing) {
    ...
    // 조건부서식은 아래에서 매번 새로 추가되므로 기존 규칙을 먼저 전부 제거한다.
    // (제거하지 않으면 동기화 1회당 2개씩 영구 누적되어 시트가 점점 느려진다)
    const prevRuleCount = (existing.conditionalFormats || []).length;
    for (let i = prevRuleCount - 1; i >= 0; i--) {
      requests.push({ deleteConditionalFormatRule: { sheetId, index: i } });
    }

    // 실데이터보다 커진 빈 그리드는 잘라낸다
    const grid = existing.properties.gridProperties || {};
    if (grid.rowCount > totalRows + 2) {
      requests.push({ deleteDimension: { range: { sheetId, dimension: 'ROWS', startIndex: totalRows + 2, endIndex: grid.rowCount } } });
    }
    if (grid.columnCount > totalCols) {
      requests.push({ deleteDimension: { range: { sheetId, dimension: 'COLUMNS', startIndex: totalCols, endIndex: grid.columnCount } } });
    }
  }
...
const { refreshAfterUpload, loadBrandDaily } = require('../agg');
...
function detectBrandFromFilename(name) {
  ...
  if (n.includes('건우') || lower.includes('gunu') || lower.includes('gunwoo') || lower.includes('geonwoo')) return '건우코리아';
  if (n.includes('아리코') || lower.includes('arico') || lower.includes('ariko')) return '아리코';
  if (n.includes('윰') || lower.includes('yum')) return '윰';
  return null;
}

/** CSV 브랜드 컬럼 값을 시트 이름과 동일한 표기로 맞춘다 */
function normalizeBrand(value) {
  const raw = value == null ? '' : String(value).trim();
  if (!raw) return '';
  return detectBrandFromFilename(raw) || raw;
}
...
function rowFromRecord(rec, fileBrand) {
  ...
  // 헤더 정규화가 '브랜드'를 brandCsv로 바꾸므로 rec.brand는 항상 비어 있다.
  const brand = fileBrand || normalizeBrand(rec.brandCsv || rec.brand || rec['브랜드']);
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST' && req.query && req.query.syncOnly === '1') {
    try {
      ...
      if (type === 'brand' && brand) {
        ...
        const SELECT = 'SELECT date, sku_id, sku_name, sales, stock, status FROM sales WHERE brand = ?';
        const ORDER = ' ORDER BY date DESC, sku_id';

        // 시트 동기화는 날짜 단위 upsert 라 이번에 올라온 날짜만 넘기면 된다.
        // 브랜드 전체 이력은 시트가 비어 복구가 필요할 때만 읽는다.
        const fetchAll = async () => shape(
          (await db.execute({ sql: SELECT + ORDER, args: [brand] })).rows);

        const isIso = (x) => /^\d{4}-\d{2}-\d{2}$/.test(String(x || ''));
        const partial = isIso(from) && isIso(to);
        let rows;
        let expectedDates = null;
        if (partial) {
          rows = shape((await db.execute({
            sql: SELECT + ' AND date >= ? AND date <= ?' + ORDER,
            args: [brand, from, to],
          })).rows);
          // 시트는 MM-DD 를 컬럼으로 쓰므로 같은 기준으로 기대 날짜 수를 센다.
          // agg_brand_daily 는 (brand, date) 라 브랜드당 수백 행만 읽는다.
          try {
            const r = await db.execute({
              sql: 'SELECT COUNT(DISTINCT substr(date, 6)) AS c FROM agg_brand_daily WHERE brand = ?',
              args: [brand],
            });
            expectedDates = Number(r.rows[0]?.c) || null;
          } catch (e) { /* 집계 테이블이 없으면 검사를 건너뛴다 */ }
        } else {
          rows = await fetchAll();
        }

        await syncBrandSheet(brand, rows, { partial, fetchAll, expectedDates });
        return res.status(200).json({ ok: true, rows: rows.length, expectedDates, message: `${brand} 시트 동기화 완료` });
      }

      if (type === 'trend') {
        const trendRows = await loadBrandDaily(db, '0000-01-01', '9999-12-31');
        const trendByBrand = {};
        for (const r of trendRows) {
          if (!trendByBrand[r.brand]) trendByBrand[r.brand] = [];
          trendByBrand[r.brand].push({ date: r.date, totalSales: Number(r.sales) || 0 });
        }
  ...
  try {
    ...
    // 집계 테이블 갱신. 실패해도 업로드 자체는 성공으로 두되 로그는 남긴다.
    try {
      await refreshAfterUpload(db, allParsedRows);
    } catch (e) {
      console.error('[UPLOAD] 집계 테이블 갱신 실패:', e.message);
    }

    // 파일명 판별에만 의존하면 이름이 바뀐 파일은 시트 동기화에서 통째로 누락된다.
    // 실제로 저장된 행의 브랜드를 그대로 돌려준다.
    const brands = [...new Set(allParsedRows.map((r) => r.brand).filter(Boolean))];

    // 시트 동기화가 브랜드 전체 이력을 읽지 않도록 이번에 처리한 날짜 범위를 알려준다.
    const brandDates = {};
    for (const r of allParsedRows) {
      if (!r.brand || !r.date) continue;
      const cur = brandDates[r.brand];
      if (!cur) brandDates[r.brand] = { from: r.date, to: r.date };
      else {
