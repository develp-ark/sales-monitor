...
const { onExcludeChanged } = require('../agg');
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // 제외 반영 집계(agg_brand_daily / agg_sku_month)를 다시 만든다.
    if (added) {
      try { await onExcludeChanged(db); }
      catch (e) { console.error('[EXCLUDE] 집계 재계산 실패:', e.message); }
    }
