...
async function syncBrandSheet(brandName, rows, opts) {
  ...
  // 증분으로 넘어왔는데 시트가 비어 있으면 과거 이력이 복구되지 않는다. 이때만 전체를 읽는다.
  if (existingRows.length < 2 && opts && opts.fetchAll) {
    const full = await opts.fetchAll();
    if (full && full.length) {
      console.log('[SHEETS] ' + brandName + ': 시트가 비어 전체 이력 ' + full.length + '행으로 복구');
      rows = full;
    }
  }
  ...
  if (req.method === 'POST' && req.query && req.query.syncOnly === '1') {
    try {
      ...
      if (type === 'brand' && brand) {
        ...
        await syncBrandSheet(brand, rows, { fetchAll });
        return res.status(200).json({ ok: true, rows: rows.length, message: `${brand} 시트 동기화 완료` });
      }
