...
module.exports = async (req, res) => {
  ...
  try {
    ...
    /* 시트 메타·URL 맵은 메인 대시보드 DB 집계와 동시에 조회해 총 대기시간 단축 */
    const sheetGidsPromise = (async () => {
      ...
      try {
        ...
        const meta = await Promise.race([
          metaPromise,
          new Promise((_, reject) => setTimeout(() => reject(new Error('sheets timeout')), 5000)),
        ]);
        for (const s of meta.data.sheets) out[s.properties.title] = s.properties.sheetId;
      } catch (e) {
        console.log('sheetGids skipped:', e.message);
      }
      return out;
    })();
    ...
    const [, allSkuDaily] = await Promise.all([
      loadWatchDaily(),
      db.execute({
        sql:
          'SELECT brand, sku_id, MAX(sku_name) AS sku_name, date, SUM(sales) AS s FROM sales WHERE date >= ? AND date <= ? GROUP BY brand, sku_id, date',
