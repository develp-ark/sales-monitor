...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const [todayAgg, sum7Agg, skuLatest, dailyTrendRows] = await Promise.all([
      ...
      db.execute({ sql: 'SELECT brand, date, SUM(sales) AS s FROM sales WHERE date >= ? AND date <= ? GROUP BY brand, date', args: [start90, latestDate] }),
    ]);
    console.log('[DASH] 4 queries:', Date.now() - t2, 'ms');
    ...
    try {
      ...
      // 여기서 실패하면 flags/skuManageMap이 조용히 비어 화면에 플래그가 사라집니다.
      console.error('[DASH] sku_manage 조회 실패 — flags/skuManageMap이 빕니다:', e.message);
    }
    ...
    const t4 = Date.now();
    const sheetGids = await sheetGidsPromise;
    console.log('[DASH] sheetGids:', Date.now() - t4, 'ms');
    ...
    const result = { brands, dailyTrend, dates, latestDate, flags, brandInsights, sheetGids, skuManageMap };
    _cache = result;
    _cacheTime = Date.now();
