...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── brandInsights: 단일 쿼리로 전체 기간 데이터 가져온 후 JS에서 분류 ──
    ...
    for (const { key, days } of PERIODS) {
      ...
      for (const [, v] of Object.entries(skuDailyMap)) {
        let cur = 0, prev = 0;
        for (const [dt, s] of Object.entries(v.dates)) {
          if (dt >= curStart && dt <= latestDate) cur += s;
          if (dt >= prevStart && dt <= prevEnd) prev += s;
        }
        ...
        surgeByBrand[v.brand].push({ sku_id: v.sku_id, sku_name: v.sku_name, cur, prev, delta: cur - prev });
      }
